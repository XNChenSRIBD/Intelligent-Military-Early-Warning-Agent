...
**部署与数据获取要求补充：** 正式系统以 `.16` 常驻采集、计算和存储为准，笔记本只作为开发与访问端。本轮 `not_local` 仅表示笔记本未找到文件，不能推定 `.16` 也缺，更不能要求用户长期手工准备本地原件。第四轮尚缺 GNSS 自动获取接线、统一下载管理前端和缓存清理；完整需求、服务器目录、保留期限与实施顺序见 [SERVER_DATA_REQUIREMENTS.md](SERVER_DATA_REQUIREMENTS.md)。

## 已准备输入及明确缺项

第五轮已在 `.16` 按清单逐项只读确认：哈尔科夫 90 份、霍尔木兹 54 份原件全部存在，合计 300,316,846 字节。下表的 `not_local` 是第四轮笔记本盘点结果，不代表服务器缺失；最新状态见 [ROUND5_DELIVERY.md](ROUND5_DELIVERY.md)。
...
144 个 GNSS 条目用于准确定位待处理的原文件，并不表示 144 个文件已在本次运行环境备齐，也不表示必须重新下载 144 个文件。应先在 `.16` 的旧 GNSS 根目录定位复用，服务器也缺的部分由系统定向获取。旧报告、旧特征 CSV 和旧模型结论不作为重新计算结果。霍尔木兹每站每天只选取名义 10:00–10:30 窗口，不宣称整日高频覆盖；名义时间须以实际 RINEX 头的时制转换为准。
...
需要 Python 3.10+，使用工作台自己的 Python 环境。以下为当前已实现回放入口的**待授权执行命令**，不是已经完成的部署记录，也尚不包含新要求的自动下载与清理。正式部署应直接使用 `.16` 已有原始数据根；系统负责定位复用和补取，保留线上 `.env`、数据库及既有模型环境。
...
Windows PowerShell 本地开发示例，从仓库根目录运行；这不是正式持续监测的存储方案：
...
剩余工作应由服务器系统承担：在 `.16` 定位和复用既有原件；接入持久下载任务，自动补取缺少的 GNSS 和霍尔木兹 PortWatch 前置数据；完成下载管理前端、GNSS 持续采集及缓存回收；按运行授权部署并执行两例正常业务流程。具体默认目录与期限见 [服务器数据需求报告](SERVER_DATA_REQUIREMENTS.md)。实际运行后记录获取/复用、处理、失败、工具调用和结果依据；当前不填造通过数量、结论或截图。
